...
## 📦 一键部署安装指南 (One-Key Global Deployment Guide)

为了让您修改后的图标与插件功能能在电脑上的各个编辑器（Antigravity、VS Code、Trae 等）中全局即时生效，本项目内置了一键部署工具。

### ⚙️ 使用说明 (How to Use)
1. **运行部署工具**：
   在工作区根目录下，找到 **`一键全局部署插件.bat`** 文件，**双击运行**即可。
   *(该工具会自动创建全局扩展软链接并清理过往旧版冲突缓存，同时配置全局文件关联与图标主题。)*
2. **重启编辑器**：
   部署成功后，关闭并**重新启动**您的开发工具（Antigravity、Trae 或 VS Code）。
3. **完成激活**：
   重启后，插件即会在所有打开的项目目录中生效，完美展现您定制的 `SCL`、`S7d`、`Stt` 蓝色高清图标！
